...
from api_keys import QWEN_API_KEY
...
if __name__ == "__main__":
    ...
    all_motions_list = sorted(os.listdir(motion_dir))[args.start:args.end]
    all_motion_caps = []
    ...
    output_file = osp.join(args.output, f'humanise_{args.task}.jsonl')
    ...
    with open(output_file, 'a+') as fo:
        for ind, motion_id in tqdm(enumerate(all_motions_list), total=len(all_motions_list)):
            ...
            scene_id = '_'.join(motion_id.split('_')[1:3])
            ...
            # randomly remove the contact with floor, since contact with floor are too dominant in the dataset
            if random.random() < 0.8:
                new_human_scene_contacts = dict()
                for k, v in human_scene_contacts.items():
                    new_v = [c for c in v if scene_object_semantics[str(c[1])] != 'floor']
                    new_human_scene_contacts[k] = new_v
                human_scene_contacts = new_human_scene_contacts
            ...
            full_motion_id = '_'.join(motion_id.split('_')[3:])
            ...
            motionindex2ann = {v["id"]: v["label"] for v in metadata}
            motion_index = int(motion_id.split('_')[0])
            action_dat = motionindex2ann[motion_index]
