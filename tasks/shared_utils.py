...
def get_dataset_attrs(datasources, attr):
    """get the a given dataset attribute for for all the dataloaders.

    Args:
        datasources (List): List of dataloaders or datasets.

    Returns: List. The dataset attributes.
    ...
    dataset_attrs = [
        getattr(dataset.datasets[0], attr)
        if isinstance(dataset, ConcatDataset)
        else getattr(dataset, attr)
...
def setup_model(
    ...
    if config.get("auto_resume", False):
        ...
        if osp.isfile(model_latest):
            config.model.pretrained_path = model_latest
            config.resume = True
        elif osp.isfile(model_best):
            config.model.pretrained_path = model_best
            config.resume = True
        else:
            logger.info(f"Not found checkpoint in {config.output_dir}")
    
    if osp.isfile(config.model.img_projector_path):
        img_projector_sd = torch.load(config.model.img_projector_path, map_location="cpu")
        msg = model_without_ddp.object_img_proj.load_state_dict(img_projector_sd)
        logger.info(f"Loaded pretrained image projector from {config.model.img_projector_path}.")

    if osp.isfile(config.model.pretrained_path):
        ...
        logger.info(msg)
        logger.info(f"Loaded checkpoint from {config.model.pretrained_path}.")
    else:
