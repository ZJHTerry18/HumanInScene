...
def shift_scale_spatemps(pred_xyzt, src_range, dst_range=None):
    """
    pred_xyzt: B x N x 4
    src_range: [[B x 4], [B x 4]] - min and max spatio-temporal input range
    dst_range: [[B x 4], [B x 4]] - min and max spatio-temporal output range
    """
    ndim = pred_xyzt.shape[-1]
    if dst_range is None:
        dst_range = [
            torch.zeros((src_range[0].shape[0], ndim), device=src_range[0].device),
            torch.ones((src_range[0].shape[0], ndim), device=src_range[0].device),
        ]

    assert src_range[0].shape[0] == pred_xyzt.shape[0]
    assert dst_range[0].shape[0] == pred_xyzt.shape[0]
    assert src_range[0].shape[-1] == pred_xyzt.shape[-1]
    assert src_range[0].shape == src_range[1].shape
    assert dst_range[0].shape == dst_range[1].shape
    assert src_range[0].shape == dst_range[1].shape
    ...
    dst_diff = dst_range[1][:, None, :] - dst_range[0][:, None, :]
    prop_xyzt = (
        ((pred_xyzt - src_range[0][:, None, :]) * dst_diff) / (src_diff + 1e-7)
    ) + dst_range[0][:, None, :]
    return prop_xyzt
