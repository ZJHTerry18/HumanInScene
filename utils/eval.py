...
def calc_scanqa_score(preds, tokenizer, scorers, config=None):
    ...
    for i, output in enumerate(preds):
        item_id = f"{output['scene_id']}_{i}"
        pred = output["qa"][0]["pred"]
        if len(pred) > 1:
            if pred[-1] == '.':
                pred = pred[:-1]
            pred = pred[0].lower() + pred[1:]
        pred = clean_answer(pred)
        ref_captions = [clean_answer(caption) for caption in output["qa"][0]["gt"]]
        ref_captions = output["qa"][0]["gt"]
