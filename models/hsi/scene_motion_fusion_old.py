...
class FusionBlock(nn.Module):
    def __init__(self, 
        ...
        # Unified self-attention for concatenated scene_f and motion_f
        self.self_attention = FusionAttention(num_heads=num_heads, hidden_dim=hidden_dim, dropout=dropout)
    ...
    def forward(self, scene_f, motion_f):
        # Concatenate scene_f and motion_f for self-attention
        sm_feat = torch.cat((scene_f, motion_f), dim=1)  # (batch_size, N1+N2, d)
        sm_feat = self.self_attention(
            query=sm_feat, 
            key=sm_feat, 
            value=sm_feat
        )
        
        # Split the self-attended result back into scene_f and motion_f
        scene_self_f, motion_self_f = torch.split(sm_feat, [scene_f.size(1), motion_f.size(1)], dim=1)
...
class SMFusion(nn.Module):
    ...
    def forward(self, scene_f, motion_f):
        for block in self.blocks:
            scene_f, motion_f = block(scene_f, motion_f)
